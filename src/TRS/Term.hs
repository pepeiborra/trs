...
-- * The class of terms
-- ----------------------------------------
class (TermShape s) => Term (t :: (* -> *) -> *) (s :: * -> *) (user :: *) | t -> user where
        isVar        :: t s -> Bool
        mkVar        :: Int -> t s
        varId        :: t s -> Maybe Int
        subTerms     :: t s -> Maybe (s(t s))
        build        :: s(t s) -> t s
...
fromSubTerms t tt = build$ modifySpine (fromMaybe (error "fromSubTerms") $ subTerms t) tt
...
mutateTerm f t  | Just tt <- subTerms t = build $ fmap f tt
                | otherwise = t
mutateTermM f t | Just tt <- subTerms t = build <$> mapM f tt
                | otherwise = return t

--instance TermShape s => Eq (TermStatic s) where
instance (Term t s user, TermShape s) => Eq (t s) where
  t1 == t2 = runST (do
...
instance (Term t s user, TermShape s) => Eq (RuleG (t s)) where
  (l1:->r1) == (l2:->r2) = runST (do
   [l1',r1',l2',r2'] <- mapM templateTerm' [l1,r1,l2,r2]
   return (isGT l1')
   liftM2 (&&) (l1' `semEq` l2') (r1' `semEq` r2'))
...
castTerm t = runST(templateGTE' t >>= zonkTerm')
...
vars t | isVar t   = [t]
       | Just tt <- toList <$> subTerms t 
                   = nubBy synEq (vars =<< tt)
       | otherwise = []

collect :: Term t s user => (t s -> Bool) -> t s -> [t s] 
collect pred t | Just tt <- toList <$> subTerms t
               = nubBy synEq ([t' | t' <- t : tt, pred t'] ++
...
templateGTE' :: Term t s user => t s -> ST r (GTE user r s)
templateGTE' = templateTerm'
